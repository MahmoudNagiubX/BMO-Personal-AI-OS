...
18. Phase 10 is the owner-authorized JARVIS Voice Core: exact local `Hey Jarvis` wake phrase using the pinned zero-cost official openWakeWord migration candidate and bounded local verifier, hands-free single-device voice on the ASUS TUF, double-Right-Ctrl activation through the same pipeline, bounded pre-roll/VAD/STT/TTS, Smart Turn, follow-up turns, and real barge-in. Push-to-talk is fallback/debug only; paid/subscription wake-word services and AccessKeys are rejected. Prior bare-`Jarvis` and other candidate results remain historical, and Phase 11 is deferred room/multi-device voice and must not start during Phase 10.
