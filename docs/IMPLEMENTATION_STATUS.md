...
- **Current branch target:** `phase-10/jarvis-voice-core`; Phase 10 v2 software work uses the zero-cost offline Vosk exact-`Jarvis` adapter, shared double-Right-Ctrl/PTT activation, bounded pre-roll, Smart Turn, and cancellable phrase TTS. The microWakeWord candidate is confirmed defective historical evidence; the short physical ASUS TUF natural-use gate remains pending after software validation and is limited to three-to-five intended wake activations plus compact representative negatives.
- **Phase 10 v2 implementation evidence:** commit `94a5b980644f4d703348d78c6e6b775d845f4fe0` passed the local software checks and synthetic Vosk benchmark; final exact-head GitHub CI is an external governance check and the owner physical gate remains pending.
...
- **Later phases authorized:** ADR-0008 historically recorded Phase 5B as `AUTHORIZED_TO_START`. Phase 6, Phase 7, Phase 8, optional Phase 8.5, and Phase 9 are merged in repository history. Phase 10 JARVIS Voice Core v2 is owner-authorized; Picovoice/Porcupine and other paid wake-word services are rejected, microWakeWord remains confirmed-defective history, and Vosk is the current free offline evaluation path. Phase 11 room/multi-device voice is `NOT_STARTED` and is not authorized by this phase.
