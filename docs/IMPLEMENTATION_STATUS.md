...
- **Later phases authorized:** ADR-0008 historically recorded Phase 5B as `AUTHORIZED_TO_START`. Phase 6, Phase 7, Phase 8, optional Phase 8.5, and Phase 9 are merged in repository history. Phase 10 JARVIS Voice Core v2 is owner-authorized; Picovoice/Porcupine and other paid wake-word services are rejected, all prior wake candidates remain historical evidence, ADR-0012 selects the BMO-owned personalized MFCC/DTW path, ADR-0013 records the blocked single-stage comparison, and ADR-0014 records the blocked two-stage cascade. Phase 11 room/multi-device voice is `NOT_STARTED` and is not authorized by this phase.
