...
- **Current branch target:** `phase-10/jarvis-voice-core`; Phase 10 software is migrating the primary hands-free phrase to exact `Hey Jarvis` through the pinned official openWakeWord artifact and bounded local verifier. The prior bare-`Jarvis` cascade and its 149/150 streaming result remain historical evidence. The new migration gate is currently blocked pending an independent held-out run; no owner physical session is authorized until its recall/FAR thresholds pass. Right-Ctrl/PTT activation, pre-roll, Smart Turn, barge-in, privacy, and Core authority remain unchanged. The earlier 0/3 bare-`Jarvis` physical result remains historical.
