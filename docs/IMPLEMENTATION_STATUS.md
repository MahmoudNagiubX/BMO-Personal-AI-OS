 ...
 **Current branch target:** `phase-10/jarvis-voice-core`; the active wake design is the product-owned Rhasspy `pyopen-wakeword==1.1.0` streaming adapter using built-in `Model.HEY_JARVIS`, with the installed model hash and upstream reference commits recorded in ADR-0022 and the Rhasspy evidence. The compact owner wake-only probe remains pending and requires no enrollment. The prior openWakeWord/verifier/Whisper wake paths and backend comparisons remain historical; faster-whisper remains conversational STT. Right-Ctrl/PTT activation, pre-roll, Smart Turn, barge-in, privacy, and Core authority remain unchanged.
...
- **Phase 10 historical extraction correction:** owner attempt 2 and its temporary-audio safeguards remain preserved as historical evidence; the active wake path uses the package-provided Rhasspy streaming feature pipeline.
...
- **Later phases authorized:** ADR-0008 historically recorded Phase 5B as `AUTHORIZED_TO_START`. Phase 6, Phase 7, Phase 8, optional Phase 8.5, and Phase 9 are merged in repository history. Phase 10 JARVIS Voice Core v2 is owner-authorized; Picovoice/Porcupine and other paid wake-word services are rejected, all prior wake candidates remain historical evidence, ADR-0012 through ADR-0021 record superseded or historical evaluations, and ADR-0022 records the active Rhasspy streaming wake reset with the compact physical probe still pending. Phase 11 room/multi-device voice is `NOT_STARTED` and is not authorized by this phase.
 **Hey Jarvis final audit:** ADR-0018 through ADR-0021 retain historical cleanup, reselection, verifier, and enrollment evidence. ADR-0022 records the active Rhasspy streaming wake reset and compact owner probe boundary. Phase 11 remains `NOT_STARTED`.
