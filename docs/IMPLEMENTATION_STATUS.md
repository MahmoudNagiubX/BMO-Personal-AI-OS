 ...
 **Current branch target:** `phase-10/jarvis-voice-core`; the active wake design is the product-owned Silero VAD -> bounded faster-whisper `base.en` CPU `int8` exact-prefix adapter recorded in ADR-0023 and its speech-gated evidence. The current bounded streaming diagnostic is below the software gate at 5/6 recall and 1/21 false activations, so the compact owner wake-only probe remains blocked and requires no enrollment. Rhasspy, openWakeWord, microWakeWord, Vosk, owner-verifier, and other prior wake paths remain historical; faster-whisper remains conversational STT after wake. Right-Ctrl/PTT activation, pre-roll, Smart Turn, barge-in, privacy, and Core authority remain unchanged. The active TUF-to-VENOM launcher uses `BMO_VENOM_HOST`, defaulting to the verified current host `192.162.1.28`, strict host-key checking, and an exact `venom-server` / `venom` identity check before opening the private Core tunnel.
- **Phase 10 full-duplex software gate:** ADR-0024 and `evidence/PHASE_10_FULL_DUPLEX_CONVERSATION.json` record the verified `JarvisConversationLoop`: one serialized final-turn worker, sleeping-only wake capture, VAD plus Smart Turn with bounded fallback, no partial Core path, cancellable TTS, bounded barge-in with playback-echo isolation, same-session follow-up, and scalar-only cleanup metrics. Exactly-once behavior is scenario/call-count proof rather than a fabricated partial-submission counter. The current targeted set passes 41/41, including the 15-scenario end-to-end conversation suite. The physical launcher now preserves dirty canonical evidence, uses a dedicated same-head resumable physical checkpoint, and reports coordinator cancellation percentiles separately from capture-start observations. Physical TUF acceptance remains pending behind the existing wake software gate; Phase 11 remains `NOT_STARTED`.
