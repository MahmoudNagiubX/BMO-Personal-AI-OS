...
- **Current branch target:** `phase-10/jarvis-voice-core`; Phase 10 v2 software work uses the zero-cost BMO-owned personalized MFCC/DTW exact-`Jarvis` adapter with derived-template-only enrollment, shared double-Right-Ctrl/PTT activation, bounded pre-roll, Smart Turn, and cancellable phrase TTS. microWakeWord, Sherpa KWS, Vosk, PocketSphinx, and the ambiguous local-wake neural embedding candidate remain historical evidence; the short physical ASUS TUF natural-use gate remains pending after software validation and is limited to three-to-five intended wake activations plus compact representative negatives.
- **Phase 10 v2 implementation evidence:** commit `c46bddba7e6f3350ba1e86d6d61959855008b85e` records the non-owner MFCC viability benchmark with 10/10 generated positives and 2/20 similar-word hard-negative false activations; this is viability evidence, not final personalized acceptance. Exact-head GitHub CI is an external governance check and the owner enrollment/physical gate remains pending.
...
- **Later phases authorized:** ADR-0008 historically recorded Phase 5B as `AUTHORIZED_TO_START`. Phase 6, Phase 7, Phase 8, optional Phase 8.5, and Phase 9 are merged in repository history. Phase 10 JARVIS Voice Core v2 is owner-authorized; Picovoice/Porcupine and other paid wake-word services are rejected, all prior wake candidates remain historical evidence, and ADR-0012 selects the BMO-owned personalized MFCC/DTW path. Phase 11 room/multi-device voice is `NOT_STARTED` and is not authorized by this phase.
