...
- **Current branch target:** `phase-10/jarvis-voice-core`; Phase 10 v2 software work uses the zero-cost BMO-owned personalized MFCC/DTW exact-`Jarvis` adapter with derived-template-only enrollment, shared double-Right-Ctrl/PTT activation, bounded pre-roll, Smart Turn, and cancellable phrase TTS. microWakeWord, Sherpa KWS, Vosk, PocketSphinx, and the ambiguous local-wake neural embedding candidate remain historical evidence; the short physical ASUS TUF natural-use gate remains paused because neither the single-stage comparison nor the bounded two-stage cascade reached the software operating point.
- **Phase 10 v2 implementation evidence:** commit `c46bddba7e6f3350ba1e86d6d61959855008b85e` records the earlier non-owner MFCC viability benchmark. The follow-up comparison runner at `a7ae0f83f9827ce6e62b10ceee8f9cf8244086e8` tested 48 positives and 248 negatives: BMO 37/48 recall with 15/248 false activations; WakeForge 48/48 recall with 248/248 false activations at its fixed threshold. The two-stage cascade runner at `b5dcd69bbd235d63f8ae0c66a2f0843428a8977c` tested 60 positives and 310 negatives with BMO→Whisper, WakeForge→Whisper, and VAD→Whisper control; each reached 56/60 (93.33%) recall with 0/310 false activations, below the required at-least-95% recall target. The dedicated verifier implementation at `e9de3ead8b1deccf67e135ab0f84e02ee805ce30` loaded the approved CUDA runtime and tested pinned tiny.en/base.en/small.en models. Its final 150-positive/1,075-negative held-out result was 96.0% recall with 4.19% FAR, including 45 assistant-playback false activations, so the verifier gate remains blocked. Owner enrollment and physical acceptance remain paused. Exact-head GitHub CI is an external governance check.
...
- **Later phases authorized:** ADR-0008 historically recorded Phase 5B as `AUTHORIZED_TO_START`. Phase 6, Phase 7, Phase 8, optional Phase 8.5, and Phase 9 are merged in repository history. Phase 10 JARVIS Voice Core v2 is owner-authorized; Picovoice/Porcupine and other paid wake-word services are rejected, all prior wake candidates remain historical evidence, ADR-0012 selects the BMO-owned personalized MFCC/DTW path, ADR-0013 records the blocked single-stage comparison, ADR-0014 records the blocked two-stage cascade, and ADR-0015 records the blocked English verifier optimization/self-trigger gate. Phase 11 room/multi-device voice is `NOT_STARTED` and is not authorized by this phase.
