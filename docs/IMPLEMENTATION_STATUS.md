...
- **Current phase boundary:** Phase 5B, Phase 6, Phase 7, Phase 8, optional Phase 8.5, and Phase 9 are merged. Phase 10 JARVIS Voice Core is owner-authorized; it has completed full automated system acceptance (19/19 gates PASS). Owner physical wake testing is truthfully recorded as WAIVED_BY_OWNER. Phase 10 is ready for final independent review. Phase 11 room/multi-device voice remains `NOT_STARTED`.
- **Current state:** PR #9 is merged and Phase 5A is closed. The Phase 1 Lenovo/VENOM repository foundation is merged. Phase 6 identity/device enrollment is merged and verified live on VENOM. Phase 7 text-first conversation and clients are implemented and merged, and verified live on VENOM over the reverse tunnel. Phase 8 tool permissions, exact-owner approvals, and audit trails are verified live on VENOM. Phase 9 Windows satellite outbound connection, allowlist execution, telemetry, file search, app open, project open, volume control, consequential workflow approval/verification, cancellation, and security boundaries were tested live on the ASUS TUF against VENOM. Phase 10 JARVIS Voice Core (speech-gated Faster-Whisper Hey Jarvis detection, Right-Ctrl double-tap, PTT, Smart Turn endpointing, full-duplex session coordinator, real barge-in TTS cancellation <= 300ms, and echo isolation) is verified via the automated 19-gate acceptance orchestrator (`scripts/system/run_full_system_acceptance.py`).
- **Current evidence:** Phase 1 physical evidence remains in `infrastructure/home_server/evidence/venom_physical_gate.json`. Sanitized Phase 9 evidence is in `docs/phase_reports/evidence/PHASE_09_WINDOWS_SATELLITE.json`. Sanitized Phase 10 evidence is in `docs/phase_reports/evidence/PHASE_10_AUTOMATED_VOICE_ACCEPTANCE.json` and `docs/phase_reports/evidence/FULL_SYSTEM_AUTOMATED_ACCEPTANCE.json`. Owner physical wake acceptance is recorded as `WAIVED_BY_OWNER` with reason: *"Owner explicitly declined additional manual wake-word trials after repeated prior physical testing and delegated remaining acceptance to automated local validation."* Historical physical wake trials remain truthfully recorded as 0/3 with 0 false activations.
- **Current branch target:** `phase-10/jarvis-voice-core`; the active wake design is the product-owned Silero VAD -> bounded faster-whisper `base.en` CPU `int8` exact-prefix adapter recorded in ADR-0023 with verified timing defaults (0.48s initial verification, 0.16s retry cadence, 8 maximum attempts up to 1.8s candidate window). Automated acceptance achieves 100% positive recall (5/5) and 0% false activation rate (0/15). Faster-whisper medium (CUDA fp16) provides conversational STT; sherpa-onnx VITS Piper provides local English and Arabic TTS. The active TUF-to-VENOM launcher uses `BMO_VENOM_HOST`, defaulting to the verified current host `192.162.1.28`, strict host-key checking, and an exact `venom-server` / `venom` identity check before opening the private Core tunnel.
- **Phase 10 full-duplex software gate:** ADR-0024 and `evidence/PHASE_10_AUTOMATED_VOICE_ACCEPTANCE.json` record the verified `JarvisConversationLoop`: one serialized final-turn worker, sleeping-only wake capture, VAD plus Smart Turn with bounded fallback, no partial Core path, cancellable TTS, bounded barge-in with playback-echo isolation, same-session follow-up, and scalar-only cleanup metrics. All 19 acceptance gates pass in `scripts/system/run_full_system_acceptance.py`. Phase 11 remains `NOT_STARTED`; PR #21 remains `OPEN / DRAFT / UNMERGED`.
- **VENOM Core recovery:** The existing baseline release `24297a9c8ce8ce8d386874949aa3d87e0881d9cc` was restored on verified host `192.162.1.28` after diagnosing `bmo-core.service` as disabled/inactive. The unit is now enabled and active; local `/health/live`, `/health/ready`, and `/version` pass, with Core bound only to `127.0.0.1:8000` and PostgreSQL remaining loopback-only on `127.0.0.1:5432`. `Linger=no` was retained because the observed outage was fixed by enabling the existing user service and the bounded SSH reconnect check retained health without a privileged lifecycle change. Exact deploy and rollback scripts now enable `bmo-core` before restart. Physical TUF acceptance remains pending; Phase 11 remains `NOT_STARTED`.
...
- **Phase 10 historical owner-verifier enrollment:** the first scalar-only owner attempt is retained truthfully in `evidence/PHASE_10_OWNER_VERIFIER.json`, but the verifier is superseded by ADR-0022 and is not in the active path. No re-enrollment is required.
- **Phase 10 historical extraction correction:** owner attempt 2 and its temporary-audio safeguards remain preserved as historical evidence; the active wake path uses the product-owned bounded Silero VAD and faster-whisper ASR pipeline.
...
- **Measured stability:** 24-hour `WAITING / WAIVED_AS_BLOCKING_PREREQUISITE / still monitoring`; seven-day `WAITING / WAIVED_AS_BLOCKING_PREREQUISITE / still monitoring`. These are not stability PASS states.
 - **Later phases authorized:** ADR-0008 historically recorded Phase 5B as `AUTHORIZED_TO_START`. Phase 6, Phase 7, Phase 8, optional Phase 8.5, and Phase 9 are merged in repository history. Phase 10 JARVIS Voice Core v2 is owner-authorized; Picovoice/Porcupine and other paid wake-word services are rejected, all prior wake candidates remain historical evidence, ADR-0012 through ADR-0022 record superseded or historical evaluations, and ADR-0023 records the active speech-gated ASR wake path. Its current bounded diagnostic is below the required software gate, so the compact physical probe is blocked. Phase 11 room/multi-device voice is `NOT_STARTED` and is not authorized by this phase.
 **Hey Jarvis final audit:** ADR-0018 through ADR-0022 retain historical cleanup, reselection, verifier, enrollment, and Rhasspy evidence. ADR-0023 records the active Silero VAD -> faster-whisper exact-prefix path and compact owner probe boundary. Phase 11 remains `NOT_STARTED`.
