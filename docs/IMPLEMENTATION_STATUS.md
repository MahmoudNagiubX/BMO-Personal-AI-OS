...
+ **Current branch target:** `phase-10/jarvis-voice-core`; the final software architecture is one official openWakeWord Hey Jarvis candidate plus a bounded local faster-whisper exact-prefix verifier. The authoritative 120-positive/3,540-negative run at `e103a62523dcfa1253c449775492e34a4497359d` recorded 91.67% recall, 0.68% FAR, and 19.67 false activations/hour, so the software gate is blocked and no owner physical session is authorized. The prior bare-`Jarvis` cascade and its 149/150 streaming result remain historical evidence. Right-Ctrl/PTT activation, pre-roll, Smart Turn, barge-in, privacy, and Core authority remain unchanged.
...
+ **Hey Jarvis final audit:** ADR-0018 records the migration and ADR-0019 records the one-backend architecture, official model identity, separate engine/pretrained-model licenses, removal of retired runnable paths, compact owner policy, and the blocked software-gate boundary. Phase 11 remains `NOT_STARTED`.
