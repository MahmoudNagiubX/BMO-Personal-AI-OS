...
- **Phase 10 full-duplex software gate:** ADR-0024 and `evidence/PHASE_10_FULL_DUPLEX_CONVERSATION.json` record the verified `JarvisConversationLoop`: one serialized final-turn worker, sleeping-only wake capture, VAD plus Smart Turn with bounded fallback, no partial Core submission, cancellable TTS, bounded barge-in, same-session follow-up, and scalar-only cleanup metrics. The synthetic targeted set passed 35/35, including the 14-scenario end-to-end conversation suite. Physical TUF acceptance remains pending behind the existing wake software gate; Phase 11 remains `NOT_STARTED`.
