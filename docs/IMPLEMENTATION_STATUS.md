...
- **Current phase boundary:** Phase 5B, Phase 6, Phase 7, Phase 8, and optional Phase 8.5 are merged. PR #19 merged Phase 8.5 at `24297a9c8ce8ce8d386874949aa3d87e0881d9cc`. Phase 9 repository implementation is tested at `6e27279b487b67a129cecab3ec56db229947e74e`; its physical gate is blocked because persistent Phase 6/7/8 Core API and PostgreSQL authority are not deployed on VENOM. Phase 10 is `NOT_STARTED`.
- **Current state:** PR #9 is merged and Phase 5A is closed. The Phase 1 Lenovo/VENOM repository foundation is merged. Phase 6 identity/device enrollment is merged. Phase 7 text-first conversation and clients are implemented and merged. Exact Phase 5B security-fix commit `e902e85dc6651d63f536a34516b31eb29c778b5c` remains deployed on VENOM. Read-only inspection on 2026-08-20 found no Phase 6, Phase 7, or Phase 8 deployment directories, no Core API unit/process/listener, and inactive PostgreSQL. Repository history contains those merged phases, but that does not constitute physical production deployment.
- **Current evidence:** Phase 1 physical evidence remains in `infrastructure/home_server/evidence/venom_physical_gate.json`. Sanitized Phase 9 repository acceptance and the physical prerequisite blocker are recorded in `docs/phase_reports/evidence/PHASE_09_WINDOWS_SATELLITE.json`.
- **Current branch target:** `phase-09/windows-satellite`; independent review is required, and the branch must not be merged while the required real TUF/VENOM tool gate is blocked.
...
- **Later phases authorized:** ADR-0008 historically recorded Phase 5B as `AUTHORIZED_TO_START`. Phase 6, Phase 7, Phase 8, and optional Phase 8.5 are merged in repository history. Phase 9 is owner-authorized and repository-tested but not physically accepted. Phase 10 is not authorized or started.
...
- The accepted sequence is **architecture update restoring Lenovo → Lenovo G450 Safety Gate → Lenovo Ubuntu Server foundation → Phase 5B deployment/integration acceptance → Phase 6 identity/device enrollment → Phase 7 → Phase 8 repository security platform → optional Phase 8.5 advanced-provider admission → Phase 9 Windows satellite**. The measured stability gates remain waiting under ADR-0008 and background monitoring remains active. Phase 9 cannot complete its physical gate until the already-merged Phase 6/7/8 authority stack is separately deployed and admitted on VENOM.
...
Phase 9 adds the repository-side authenticated outbound Windows satellite, strict local allowlist, fixed typed executors, secure current-user credential storage, cancellation/replay protection, and Phase 8-governed routing. It does not add an inbound TUF listener, public/LAN command endpoint, general shell, remote allowlist mutation, heavy Lenovo compute, or Phase 10 work. Physical Phase 9 acceptance remains blocked at the explicit prerequisite boundary; no missing historical production phase was silently deployed. Background Phase 1 monitoring remains actionable: SMART overall failure, any SMART counters 5/197/198 above zero, repeated thermal breach, root-filesystem pressure, unexpected reboot patterns, repeated failed units, or repeated Ethernet management-path loss pause deployment expansion and require reporting.
