...
- **Current branch target:** `phase-10/jarvis-voice-core`; Phase 10 software is migrating the primary hands-free phrase to exact `Hey Jarvis` through the pinned official openWakeWord artifact and bounded local verifier. The independent 120-positive/3,540-negative held-out migration run at `e103a62523dcfa1253c449775492e34a4497359d` recorded 91.67% recall, 0.68% FAR, and 19.67 false activations/hour, so the migration software gate is blocked and no owner physical session is authorized. The prior bare-`Jarvis` cascade and its 149/150 streaming result remain historical evidence. Right-Ctrl/PTT activation, pre-roll, Smart Turn, barge-in, privacy, and Core authority remain unchanged. The earlier 0/3 bare-`Jarvis` physical result remains historical.
...
- **Hey Jarvis migration:** ADR-0018 records the owner-authorized exact `Hey Jarvis` phrase, official Apache-2.0 openWakeWord artifact identity, historical bare-`Jarvis` evidence preservation, compact owner policy, and the blocked software-gate boundary. Phase 11 remains `NOT_STARTED`.
