...
+ Phase 10 uses one normal path for the exact `Hey Jarvis` phrase: the pinned official openWakeWord `hey_jarvis_v0.1.onnx` candidate followed by a bounded local faster-whisper exact-prefix verifier, rolling Silero VAD/local Smart Turn, authenticated Core API/agent, and safe phrase/sentence sherpa-onnx TTS. ADR-0018 records the migration and ADR-0019 records the final one-backend cleanup, separate Apache engine/CC-BY-NC-SA-4.0 pretrained-model licensing, temporal policy, and rollback. The independent 120-positive/3,540-negative run at `e103a62523dcfa1253c449775492e34a4497359d` recorded 91.67% recall, 0.68% FAR, and 19.67 false activations/hour, so the software gate is blocked and no owner physical session is authorized. Bare `Jarvis`, MFCC, WakeForge, microWakeWord, Sherpa KWS, Vosk, PocketSphinx, and other candidate results remain historical evidence only.
