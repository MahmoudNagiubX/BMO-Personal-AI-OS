...
- Phase 10 is migrating the normal path to the exact `Hey Jarvis` phrase through the pinned official openWakeWord `hey_jarvis_v0.1.onnx` adapter, with an optional bounded local faster-whisper verifier, rolling Silero VAD/local Smart Turn, authenticated Core API/agent, and safe phrase/sentence sherpa-onnx TTS. ADR-0018 records the artifact provenance, Apache-2.0 license, checksum, and software-gate thresholds. The independent 120-positive/3,540-negative migration run at `e103a62523dcfa1253c449775492e34a4497359d` recorded 91.67% recall, 0.68% FAR, and 19.67 false activations/hour, so the migration software gate is blocked and no owner physical session is authorized. The prior bare-`Jarvis`, MFCC, WakeForge, microWakeWord, Sherpa KWS, Vosk, PocketSphinx, and other candidate results remain historical evidence.
